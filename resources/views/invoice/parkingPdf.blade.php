...
$parking = isset($invoiceMeta['parking']) && !is_null($invoiceMeta['parking'])
? json_decode($invoiceMeta['parking'], true) ?? []
: [];
...
$totalRoutesAmount = array_sum(array_column($parking, 'parking_amount'));
    ...
    <div style="max-width: 1236px; margin: 0 auto; padding: 20px;">
        <table style="max-width: 100%; width: 100%; margin-top: 10px; text-align: left;" cellpadding="0" cellspacing="0">
            <tbody>
                ...
                <tr>
                    <td>
                        <h2 style="font-size: 25px; color: rgba(0, 0, 0, 0.87); margin: 0 0 4px;">{{$company['name'] }}</h2>
                        ...
                        <p style="font-size: 19px; color: rgba(0, 0, 0, 0.87); margin-top: 0px; padding-top:20px; font-weight: 600;">#Due Date: <span style="font-weight: 400;">{{$invoiceData['due_date']}}</span></p>
                    </td>
                    <td></td>
                    <td>
                ...
                <tr>
                    <td style="font-size: 21px; color:rgba(0, 0, 0, 0.87); margin-top:70px; text-align:left;font-weight:600;">Invoice Details </td>
                    <td style="font-size: 19px; color:rgba(0, 0, 0, 0.87); margin-top:70px; text-align: center; font-weight:600;"></span></td>
                    <td style="font-size: 19px; color:rgba(0, 0, 0, 0.87); margin-top:70px; text-align: center; font-weight:600;"></span></td>
                ...
                <tr>
                    <td colspan="3">
                        <table cellpadding="8" style="width:100%; margin-top:18px; text-align:left !important;" cellpadding="0" cellspacing="0">
                            <thead>
                            
                                <tr>
                                    <th style="background: #0c2345; font-size: 19px; color: #fff; padding: 8px 7px; text-align: left;">Sr. No.</th>
                                    <th style="background: #0c2345; font-size: 19px; color: #fff; padding: 8px 7px; text-align: left;">Particulars</th>
                                    <th style="background: #0c2345; font-size: 19px; color: #fff; padding: 8px 7px; text-align: right;">Amount($)</th>
                                </tr>
                            </thead>
                            <tbody>
                            @for ($i = 0; $i < count($parking); $i++)
                                <tr>
                                    <td valign="top" style="border-bottom: 2px solid #c4c4c4; padding: 5px 0px;padding-left:10px;font-size:19px;width:20%">
                                    {{$i+1}}
                                    </td>
                                    <td valign="top" style="border-bottom: 2px solid #c4c4c4; padding: 5px 0px;font-size:19px;width:40%">
                                    {{$parking[$i]['particulars']}}
                                    </td>
                                    <td valign="top" style="border-bottom: 2px solid #c4c4c4; padding: 5px 0px;padding-right:10px;font-size:19px;width:40%; text-align: right;">
                                    @if(isset($parking[$i]['parking_amount']) && !is_null($parking[$i]['parking_amount']))
                                        {{ number_format($parking[$i]['parking_amount'], 2, '.', ',') }}
                                        @else
                                        {{ '0.00' }}
                                        @endif
                                    </td>
                    ...
                    <td>
                        <table style="max-width: 100%; width: 100%; margin-top: 10px; text-align: left;" cellpadding="0" cellspacing="0">
                            <tbody>
                                ...
                                @for ($i = 0; $i < count($parking); $i++)
                                    <tr>
                                    <td style="background-color: #f1f8fb; padding: 6px 12px; border-radius: 5px 0 0 0; color: #26ace4; font-size: 19px; font-weight: 600; text-transform: capitalize;">Sub Total</td>
                ...
                @for ($i = 0; $i < count($invoiceTax); $i++)
                    ...
                    <tr>
                        ...
                        <td style="background-color: #f1f8fb; padding: 6px 12px; border-radius: 0 5px 0 0; font-size: 19px; text-align: right;">{{$grand_total_amount}}</td>
                    </tr>
            </tbody>
        </table>
        </td>
        ...
        <tr>
            ...
            @if($invoiceData['is_term_condition'] === 1)
                ...
                @endif
                <!-- <table style="max-width: 100%; width: 100%; position: relative; left:0; top:30%; text-align: left;">
                    <tr>
                        <td style="width: 33.33%; vertical-align: top;">
                            <table style="max-width: 100%; width: 100%; margin-top: 10px; text-align: left;" cellpadding="0" cellspacing="0">
                                <tbody>
